...
class Regex:
    ...
    def _to_cfg(self, ast, count): 
        cfg_rules = []
        my_rule = {
            'Name': f'_{self.name}_{count}',
            'Squash': True,
            'Is': []
        }
        cfg_rules += [my_rule]
        ...
        # OR IS SPECIAL: by nature, it can result in first set issues in the parsing stage.
        # when the parser is being created, this needs to be taken into account by
        # not only checking first sets, but in the case where the first sets over lap, attempting
        # one option, catching the failure, and then attempting the other. This is not ideal but will 
        # allow us to avoid using regular expressions all together
        if ast.type == self._Types.OR:  
            left_rules, count_after_left = self._to_cfg(ast.children[0], count)
            right_rules, count_after_right = self._to_cfg(ast.children[1], count_after_left)

            my_rule['Is'].append([
                {
                    'Type': 'RULE',
                    'Data': f'_{self.name}_{count}'
                }
            ])
            ...
            my_rule['Is'].append([
                {
                    'Type': 'RULE',
                    'Data': f'_{self.name}_{count_after_left}'
                }
            ])

            cfg_rules += left_rules
            cfg_rules += right_rules
            count = count_after_right
            ...
            if ast.data == '*':
                ...
                my_rule['Is'].append([
                    ...
                    {
                        'Type': 'RULE',
                        'Data': f'_{self.name}_{count - 1}'
                    }
                ])
                
                cfg_rules += left_rules
                count = count_after_left
            elif ast.data == '?':
                left_rules, count_after_left = self._to_cfg(ast.children[0], count)
                ...
                if len(parameters) == 1:
                    ...
                    if parameters[1] == None:
                        ...
                        helper_rule = {
                            'Name': f'_{self.name}_{count_after_left}',
                            'Squash': True,
                            'Is': []
                        }

                        helper_rule['Is'].append([])
                        helper_rule['Is'].append([
                            {
                                'Type': 'RULE',
                                'Data': f'_{self.name}_{count}'
                        ...
                        cfg_rules += [helper_rule]
                        cfg_rules += left_rules
                        count = count_after_left + 1
            ...
            curr = tokens
            last = self._Token(':)', curr)
            while curr != None:
                if curr.value != '-':
                    if curr.next != None and curr.next.value != '-' and last.value != '-':
                        curr.next = self._Token('-', self._Token(curr.value, curr.next))
                        last = curr.next.next
                        curr = curr.next.next.next
                    elif curr.next == None and last.value != '-':
                        curr.next = self._Token('-', self._Token(curr.value, curr.next))
                        last = curr.next.next
                        curr = curr.next.next.next
                    else:
                ...
                if value == '-' and last_value == '-':
                    ...
                    if len(value) == 2:
                        if value[1] in ['w', 'W', 'd', 'D', 's', 'S', '.']:
                            ...
                            ranges += [(pair_first, value[1])]
                    else:
                        ranges += [(pair_first, value)]
                elif value != '-':
                    if len(value) == 2:
                        if value[1] in ['w', 'W', 'd', 'D', 's', 'S', '.']:
            ...
            my_rule['Is'].append([
                {
                    'Type': '+ RANGE' if is_affirmative else '- RANGE',
                    'Data': [
                        [t[0], t[1]] for t in ranges
                    ]
                }
            ])

        elif ast.type == self._Types.UNIT:
            if ast.data[0] == '\\':
                if ast.data[1] == 's':
                    my_rule['Is'].append([
                        {
        ...
        return cfg_rules, count

    # convert the AST to the appropriate output JSON
    def to_cfg(self):
        ...
        new_rules, _ = self._to_cfg(ast, 0)
        cfg_rules += new_rules
        return cfg_rules
    ...
    class _Range:
        ...
        def _to_range(value):
            if value == '\\d':
                return Regex._Range(frozenset([('0', '9')]), True)
            elif value == '\\D':
                return Regex._Range(frozenset([('0', '9')]), False)
            elif value == '\\w':
                return Regex._Range(frozenset([('a', 'z'), ('A', 'Z'), ('0', '9'), ('_', '_')]), True)
            elif value == '\\W':
                return Regex._Range(frozenset([('a', 'z'), ('A', 'Z'), ('0', '9'), ('_', '_')]), False)
            elif value == '\\s':
                return Regex._Range(frozenset([('\t', '\t'), ('\n', '\n'), ('\r', '\r'), (' ', ' ')]), True)
            elif value == '\\S':
                return Regex._Range(frozenset([('\t', '\t'), ('\n', '\n'), ('\r', '\r'), (' ', ' ')]), False)
            elif value == '\\.':
                return Regex._Range(frozenset([(chr(0), chr(127))]), True)
            else:
                return Regex._Range(frozenset([(value[1], value[1])]), True)
    ...
    def _ast_to_nfa(self, ast):
        ...
        #          o
            ...
            our_range = self._Range(frozenset([]), is_affirmative)
            tokens = self._pre_lex(parameters)
            last_value = ''
            pair_first = None
            curr = tokens
                ...
                if value == '-' and last_value == '-':
                    ...
                    if len(value) == 2:
                        if value[1] in ['w', 'W', 'd', 'D', 's', 'S']:
                            ...
                            our_range._add_range(self._Range(frozenset([(pair_first, value[1])]), is_affirmative))
                    else:
                        our_range._add_range(self._Range(frozenset([(pair_first, value)]), is_affirmative))
                elif value != '-':
                    if len(value) == 2:
                        if value[1] in ['w', 'W', 'd', 'D', 's', 'S']:
                            raise Exception('Parse Error: Use special characters outside of grouping.')
                        else:
                            our_range._add_range(self._Range(frozenset([(value[1], value[1])]), is_affirmative))
                    else:
                        our_range._add_range(self._Range(frozenset([(value, value)]), is_affirmative))
        ...
        #
            ...
            if ast.data[0] == '\\':
                start._add_transition(self._Range._to_range(ast.data), end)
            else:
                start._add_transition(self._Range(frozenset([(ast.data, ast.data)]), True), end)
    ...
    def _to_dfa(self, nfa):
        pass

    def compile(self):
        nfa = self._to_nfa()
        dfa = self._to_dfa(nfa)
    '''

# cfg = Regex('MyRegex', '[ba-zxb-j]').to_cfg()
# pprint.pprint(cfg)